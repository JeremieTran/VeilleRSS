...
async function readDirectoryAsync(options, targetDirectoryPath) {
    const receivedItemsPaths = await node_fs_1.promises.readdir(targetDirectoryPath);
    const result = await Promise.all(receivedItemsPaths.map(async (itemPath) => {
        const receivedFilesPaths = [];
        const relativeItemPath = `${targetDirectoryPath}/${itemPath}`;
        const stat = await node_fs_1.promises.stat(relativeItemPath);
...
exports.default = readDirectoryAsync;
